...
class MovementSystem(entityManager: EntityManager, world: World) : EntitySystem(entityManager) {
	...
	private fun moveLimbsToTransform(entity: Entity) {
	...
	private fun getY(limbsPart: LimbsPart): Float {
		...
		return descendants.map { it.transform.bounds.y }.minBy { it } ?: Float.NaN
	}
}